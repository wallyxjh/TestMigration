...
# 设置最新版本的环境变量（请替换为实际的最新版本号）
ENV MYSQL_VERSION latest
ENV ZOOKEEPER_VERSION latest
ENV HADOOP_VERSION latest
ENV HIVE_VERSION latest
...
# 安装 MySQL
RUN wget https://dev.mysql.com/get/Downloads/MySQL-${MYSQL_VERSION}/mysql-${MYSQL_VERSION}-linux-glibc2.12-x86_64.tar.xz
